...
class Role(db.Model):
    ...
    created_at = db.Column(db.DateTime, nullable=False, default=datetime.now(ZoneInfo("US/Central")))
    active = db.Column(db.Boolean, nullable=False, default=False)

    # Relationships
    users = db.relationship("User", back_populates="role")
    role_policies = db.relationship('RolePolicy', back_populates='role', cascade='all, delete-orphan')
...
class Policy(db.Model):
    ...
    # Relationships
    role_policies = db.relationship('RolePolicy', back_populates='policy', cascade='all, delete-orphan')
    policy_permissions = db.relationship('PolicyPermission', back_populates='policy', cascade='all, delete-orphan')

    # permissions = db.relationship('PolicyPermission', back_populates='policy', cascade='all, delete-orphan')

    def has_permission(self, permission_name: str) -> bool:
        return any(pp.permission.name == permission_name for pp in self.policy_permissions)
...
class Permission(db.Model):
    ...
    policy_permissions = db.relationship('PolicyPermission', back_populates='permission', cascade='all, delete-orphan')
